...
- commits:
    - subject: Deprecate `prepare` without expected parameter aliases
      ...
      author: Pagan Gazzard
      nested: []
    - subject: Add the option to list expected parameter aliases when preparing a query
...
- commits:
    - subject: Change `ExpandPropsOf` and `ExpandToResponse` helpers to check the
      ...
      body: |
        This reduces the amount of work typescript has to do in the common case
      footer:
        Change-type: patch
        change-type: patch
      author: Pagan Gazzard
...
- commits:
    - subject: Deprecate passing `url` to helper functions
      ...
      body: |
        The intention is that these are use as helpers functions to build the
        url from an object and so passing a `url` directly is not really the
        intent and interferes with improving the typings for the intended case.
        It is still possible to pass urls directly to the `request` function if
        it is essential but the true answer is to support building those urls,
...
- commits:
    - subject: Type `upsert` and `getOrCreate` as not accepting a `url`
      ...
      body: |
        This is because they are not designed to support using a `url` and are
        likely to break in very weird and unexpected ways, and as such removing
...
- commits:
    ...
    - author: Pagan Gazzard
      ...
      subject: Remove deprecated automatic $ prefixing for `options`, and deprecate
        `customOptions`
    - author: Pagan Gazzard
      body: ""
      footers:
      ...
      subject: Move type dependencies from dev to normal dependencies, so they are
    ...
    - author: Pagan Gazzard
      ...
      subject: Port "Add 'subscribe' method for repeated requests for the same resource"
    - author: Pagan Gazzard
      body: ""
      footers:
        change-type: major
    ...
    - author: Pagan Gazzard
      ...
      subject: Switch to a named `PinejsClientCoreFactory` export, rather than
    ...
    - author: Pagan Gazzard
      ...
      subject: Switch to `Array.prototype.map` dropping support for some very old
        browsers.
    - author: Pagan Gazzard
      body: ""
      footers:
      ...
      subject: Stop requiring that an `isArray` function be provided and instead use
    ...
    - author: Pagan Gazzard
      ...
      subject: Stop requiring that an `isNumber` function be provided and instead use
    ...
    - author: Pagan Gazzard
      ...
      subject: Stop requiring that an `isString` function be provided and instead use
        a simple check that requires only primitive strings be used (not wrapped
        in a String object)
    - author: Pagan Gazzard
      body: ""
      footers:
      ...
      subject: Stop requiring that an `isBoolean` function be provided and instead use
    ...
    - author: Pagan Gazzard
      ...
      subject: Stop requiring a utils object be provided as the final `isObject`
    ...
    - author: Pagan Gazzard
      ...
      subject: Change to the subscribe pollInterval to be the amount of time between
...
- commits:
    - author: Pagan Gazzard
      ...
      subject: Fix expand typings, by making sure that the ODataOptions are only valid
...
- commits:
    - author: Pagan Gazzard
      ...
      subject: Fix typing for request, previously no class type was defined due to
...
- commits:
    - author: Pagan Gazzard
      ...
      subject: Allow passing in the promise types in order to get a correctly typed
        promise returned
    - author: Pagan Gazzard
      body: ""
      footers:
...
- commits:
    ...
    - author: Pagan Gazzard
      ...
      subject: Export the AnyObject type so it can be used for typing that we know the
    ...
    - author: Pagan Gazzard
      ...
      subject: Merge properties into FilterObj directly, as the & merging doesn't work
        as expected
    - author: Pagan Gazzard
      body: ""
      footers:
      ...
      subject: Export a lot more of the types so they can be used by external typescript
    - author: Pagan Gazzard
      body: ""
      footers:
        change-type: patch
...
- commits:
    - author: Pagan Gazzard
      ...
      subject: Allow options starting with $ to be used, and deprecating options that
...
- commits:
    - author: Pagan Gazzard
      ...
      subject: Add support for properly validating the contents of `$orderby`,
        `$select`, `$skip`, and `$top`
    - author: Pagan Gazzard
      body: ""
