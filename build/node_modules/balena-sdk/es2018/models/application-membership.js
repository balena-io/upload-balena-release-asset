...
const getApplicationMembershipModel = function (deps, getApplication) {
    const { pine } = deps;
    ...
    const exports = {
        ...
        async get(membershipId, options = {}) {
            ...
            if (result == null) {
                throw new errors.BalenaError(`Application Membership not found: ${membershipId}`);
            }
            return result;
        },
        /**
        ...
        async getAllByApplication(slugOrUuidOrId, options = {}) {
            ...
            return await pine.get({
                resource: RESOURCE,
                options: (0, util_1.mergePineOptions)({ $filter: { is_member_of__application: id } }, options),
            });
        },
        /**
         ...
         * });
         */
        async getAllByUser(usernameOrId, options = {}) {
            if (typeof usernameOrId !== 'number' &&
                typeof usernameOrId !== 'string') {
                throw new errors.BalenaInvalidParameterError('usernameOrId', usernameOrId);
            }
            return await pine.get({
                resource: RESOURCE,
                options: (0, util_1.mergePineOptions)({
                    $filter: {
                        user: typeof usernameOrId === 'number'
                            ? usernameOrId
                            : {
                                $any: {
                                    $alias: 'u',
         ...
         * });
         */
        async create({ application, username, roleName, }) {
            const appOptions = {
                $select: 'id',
                $expand: {
                    organization: {
            ...
            if (organization[0].organization_membership.length === 0) {
                throw new Error('It is necessary that each user (Auth) that is member of an application that has an organization, is member of the organization');
            }
            const body = {
                username,
         ...
         * });
         */
        async changeRole(idOrUniqueKey, roleName) {
