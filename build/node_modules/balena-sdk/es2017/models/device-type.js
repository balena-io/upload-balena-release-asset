    ...
    for (const partialKey of Object.keys(partials)) {
        ...
        if (Array.isArray(current)) {
            ...
            location[partialKey] = current
                .flatMap((partial) => Handlebars.compile(partial)(interpolated, handlebarsRuntimeOptions).split(`\n`))
                .map((n) => n.trim())
                .filter((n) => n);
        }
        else {
            // if it's another dictionary, keep traversing
            interpolated = traversingCompile(current, interpolated, path.concat([partialKey]));
        }
    }
    return interpolated;
};
const interpolatedPartials = (contract) => {
...
const calculateInstallMethod = (contract) => {
    var _a, _b, _c;
    const flashProtocol = (_a = contract.data) === null || _a === void 0 ? void 0 : _a.flashProtocol;
    const defaultBoot = (_c = (_b = contract.data) === null || _b === void 0 ? void 0 : _b.media) === null || _c === void 0 ? void 0 : _c.defaultBoot;
    if (flashProtocol) {
        ...
        throw new errors.BalenaError(`Unable to determine installation method for contract: ${contract.slug}`);
    }
};
function getInstructionsFromContract(contract) {
    var _a;
    const installMethod = calculateInstallMethod(contract);
    const interpolatedDeviceType = {
        deviceType: interpolatedPartials(contract),
    };
    const interpolatedHostOS = interpolatedPartials(Object.assign(Object.assign({}, (0, cloneDeep_1.default)(balenaos_contract_1.BalenaOS)), interpolatedDeviceType));
    return (_a = interpolatedHostOS.partials) === null || _a === void 0 ? void 0 : _a[installMethod];
}
const getDeviceTypeModel = function (deps) {
    const { pine } = deps;
    const exports = {
        /**
        ...
        getInstructions: async (deviceTypeSlugOrContract) => {
            ...
            if (typeof deviceTypeSlugOrContract === 'string') {
                ({ contract } = await exports.getBySlugOrName(deviceTypeSlugOrContract, {
                    $select: 'contract',
                }));
