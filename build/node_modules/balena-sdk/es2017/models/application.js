...
const getApplicationModel = function (deps, opts) {
    ...
    const membershipModel = require('./application-membership').default(deps, (...args) => exports.get(...args));
    ...
    const batchApplicationOperation = (0, once_1.default)(() => require('../util/request-batching').batchResourceOperationFactory({
    ...
    const tagsModel = buildDependentResource({ pine }, {
        ...
        async getResourceId(slugOrUuidOrId) {
            if (typeof slugOrUuidOrId !== 'string' &&
                typeof slugOrUuidOrId !== 'number') {
                throw new Error(`Unexpected type for slugOrUuidOrId provided in application tagsModel getResourceId: ${typeof slugOrUuidOrId}`);
            }
    ...
    const configVarModel = buildDependentResource({ pine }, {
        ...
        async getResourceId(slugOrUuidOrId) {
            if (typeof slugOrUuidOrId !== 'string' &&
                typeof slugOrUuidOrId !== 'number') {
                throw new Error(`Unexpected type for slugOrUuidOrId provided in application configVarModel getResourceId: ${typeof slugOrUuidOrId}`);
            }
            const { id } = await exports.get(slugOrUuidOrId, { $select: 'id' });
            return id;
        },
    ...
    const envVarModel = buildDependentResource({ pine }, {
        ...
        async getResourceId(slugOrUuidOrId) {
            if (typeof slugOrUuidOrId !== 'string' &&
                typeof slugOrUuidOrId !== 'number') {
                throw new Error(`Unexpected type for slugOrUuidOrId provided in application envVarModel getResourceId: ${typeof slugOrUuidOrId}`);
            }
            const { id } = await exports.get(slugOrUuidOrId, { $select: 'id' });
            return id;
        },
    ...
    const buildVarModel = buildDependentResource({ pine }, {
        ...
        async getResourceId(slugOrUuidOrId) {
            if (typeof slugOrUuidOrId !== 'string' &&
                typeof slugOrUuidOrId !== 'number') {
                throw new Error(`Unexpected type for slugOrUuidOrId provided in application buildVarModel getResourceId: ${typeof slugOrUuidOrId}`);
            }
            const { id } = await exports.get(slugOrUuidOrId, { $select: 'id' });
            return id;
        },
    ...
    const getId = async (slugOrUuidOrId) => {
        if ((0, util_1.isId)(slugOrUuidOrId)) {
    ...
    const exports = {
        ...
        async getAll(options, context) {
            const apps = await pine.get({
                ...
                options: (0, util_1.mergePineOptions)(Object.assign(Object.assign({}, (context === 'directly_accessible' && {
                    $filter: isDirectlyAccessibleByUserFilter,
                })), { $orderby: 'app_name asc' }), options !== null && options !== void 0 ? options : {}),
            });
            return apps;
        },
        /**
         ...
         * });
         */
        async getAllDirectlyAccessible(options) {
         ...
         * });
         */
        async getAllByOrganization(orgHandleOrId, options) {
            const { id: orgId } = await sdkInstance.models.organization.get(orgHandleOrId, {
                $select: 'id',
            });
            const apps = await pine.get({
                resource: 'application',
         ...
         * });
         */
        async get(slugOrUuidOrId, options, context) {
            options !== null && options !== void 0 ? options : (options = {});
            const accessFilter = context === 'directly_accessible'
            ...
            if ((0, util_1.isId)(slugOrUuidOrId)) {
                application = await pine.get({
                    resource: 'application',
                    id: slugOrUuidOrId,
                    options: (0, util_1.mergePineOptions)(accessFilter != null ? { $filter: accessFilter } : {}, options),
                });
            }
                ...
                const applications = await pine.get({
                    resource: 'application',
                    options: (0, util_1.mergePineOptions)({
                        $filter: Object.assign(Object.assign({}, accessFilter), { $or: {
         ...
         * });
         */
        async getDirectlyAccessible(slugOrUuidOrId, options) {
         ...
         * })
         */
        async getWithDeviceServiceDetails(slugOrUuidOrId, options) {
            options !== null && options !== void 0 ? options : (options = {});
            const serviceOptions = (0, util_1.mergePineOptions)({
                $expand: [
                    {
                        owns__device: {
                            $expand: (0, device_service_details_1.getCurrentServiceDetailsPineExpand)(true),
                        },
                    },
                ],
            }, options);
            ...
            if (app.owns__device) {
                app.owns__device = app.owns__device.map((d) => (0, device_service_details_1.generateCurrentServiceDetails)(d));
            }
            return app;
        },
        /**
         ...
         * });
         */
        async getAppByName(appName, options, context) {
            options !== null && options !== void 0 ? options : (options = {});
            const accessFilter = context === 'directly_accessible'
                ? isDirectlyAccessibleByUserFilter
                : null;
            const applications = await pine.get({
                resource: 'application',
                options: (0, util_1.mergePineOptions)({
                    $filter: Object.assign(Object.assign({}, accessFilter), { app_name: appName }),
                }, options),
            });
         ...
         */
        hasAny: async () => {
            const applications = await exports.getAll({ $select: ['id'] }, 'directly_accessible');
         ...
         * });
         */
        async create({ name, uuid, applicationClass, deviceType, organization, }) {
            ...
            const organizationPromise = pine
                ...
                id: {
                    [(0, util_1.isId)(organization) ? 'id' : 'handle']: organization,
                },
                options: {
                    $select: ['id'],
                },
