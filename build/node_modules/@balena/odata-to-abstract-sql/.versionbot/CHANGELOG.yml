...
- commits:
    - subject: Support bind references as boolean types
      ...
      body: |
        This allows things like `$filter=true` and `$filter=x/any(y:true)` to
        work
      footer:
        Change-type: minor
        change-type: minor
...
- commits:
    - subject: Cache the `ResourceMapping` result to avoid unnecessary work
      ...
      body: |
        This function can get called a lot so caching it should provide a
...
- commits:
    - subject: Always replace the table being modified in definitions, ignoring aliases
...
- commits:
    - subject: Switch insert casts from using legacy `'Null'` to using a `['Null']` node
