...
exports.OsVariant = exports.OsTypes = void 0;
...
var OsVariant;
(function (OsVariant) {
    OsVariant["production"] = "prod";
    OsVariant["development"] = "dev";
})(OsVariant || (exports.OsVariant = OsVariant = {}));
...
const getOsAppTags = (applicationTags) => {
    var _a;
    const tagMap = tagsToDictionary(applicationTags);
    return {
        osType: (_a = tagMap[RELEASE_POLICY_TAG_NAME]) !== null && _a !== void 0 ? _a : OsTypes.DEFAULT,
...
const getOsVersionReleaseLine = (phase, version, appTags) => {
    if (phase == null) {
        // All patches belong to the same line.
        if (appTags.nextLineVersionRange &&
            bSemver.satisfies(version, `^${appTags.nextLineVersionRange}`)) {
            return 'next';
        }
        if (appTags.currentLineVersionRange &&
            bSemver.satisfies(version, `^${appTags.currentLineVersionRange}`)) {
            return 'current';
        }
        if (appTags.sunsetLineVersionRange &&
...
const getOsModel = function (deps, opts) {
    ...
    const _getOsVersions = async (deviceTypes, options) => {
        return await pine.get({
            ...
            options: {
                ...
                $expand: {
                    ...
                    owns__release: (0, util_1.mergePineOptionsTyped)(baseReleasePineOptions, options),
                },
                $filter: {
                    is_host: true,
                    is_for__device_type: {
    ...
    const _getOsVersionsFromReleases = (releases, appTags) => {
        var _a, _b;
        ...
        for (const release of releases) {
            ...
            const line = (_a = getOsVersionReleaseLine(release.phase, strippedVersion, appTags)) !== null && _a !== void 0 ? _a : undefined;
            results.push(Object.assign(Object.assign({}, release), { 
                // TODO: Drop the explicit assignment once the variant field of all OS releases is backfilled.
                variant, osType: appTags.osType, line,
                strippedVersion, basedOnVersion: (_b = tagMap[BASED_ON_VERSION_TAG_NAME]) !== null && _b !== void 0 ? _b : strippedVersion }));
        }
        return results;
    };
    const _transformHostApps = (apps) => {
        const osVersionsByDeviceType = {};
        apps.forEach((hostApp) => {
            var _a, _b, _c, _d;
            const hostAppDeviceType = (_a = hostApp.is_for__device_type[0]) === null || _a === void 0 ? void 0 : _a.slug;
            if (!hostAppDeviceType) {
                return;
            }
            (_b = osVersionsByDeviceType[hostAppDeviceType]) !== null && _b !== void 0 ? _b : (osVersionsByDeviceType[hostAppDeviceType] = []);
    ...
    const _getAllOsVersions = async (deviceTypes, options, convenienceFilter) => {
        const extraFilterOptions = convenienceFilter === 'supported' || convenienceFilter === 'include_draft'
            ? {
                $filter: Object.assign(Object.assign({}, (convenienceFilter === 'supported' && { is_final: true })), { is_invalidated: false, status: 'success' }),
            }
            : undefined;
        const finalOptions = options != null
            ? (0, util_1.mergePineOptions)(options, extraFilterOptions)
            : extraFilterOptions;
        const hostapps = await _getOsVersions(deviceTypes, finalOptions);
        return _transformHostApps(hostapps);
    };
    const _memoizedGetAllOsVersions = authDependentMemoizer(async (deviceTypes, convenienceFilter) => {
     ...
     */
    async function getAvailableOsVersions(deviceTypes, 
    ...
    options) {
        var _a;
        const pineOptionEntries = options != null
            ? Object.entries(options).filter(([key]) => key.startsWith('$'))
            : undefined;
        const pineOptions = pineOptionEntries != null && pineOptionEntries.length > 0
            ? Object.fromEntries(pineOptionEntries)
            : undefined;
        ...
        const convenienceFilter = (options === null || options === void 0 ? void 0 : options.includeDraft) === true ? 'include_draft' : 'supported';
        const versionsByDt = (pineOptions == null
            ? await _memoizedGetAllOsVersions(deviceTypes.slice().sort(), convenienceFilter)
     ...
     */
    async function getAllOsVersions(deviceTypes, options) {
        var _a;
        const singleDeviceTypeArg = typeof deviceTypes === 'string' ? deviceTypes : false;
        deviceTypes = Array.isArray(deviceTypes) ? deviceTypes : [deviceTypes];
     ...
     * @memberof balena.models.os
     */
    const _getNormalizedDeviceTypeSlug = authDependentMemoizer(async (deviceTypeSlug) => {
    ...
    const _getMaxSatisfyingVersion = function (versionOrRange, osVersions) {
        ...
        if (versionOrRange === 'latest') {
            return (_a = osVersions[0]) === null || _a === void 0 ? void 0 : _a.raw_version;
        }
        // TODO-next-major: Drop the `default` parameter value
        if (versionOrRange === 'default') {
            return (_b = osVersions[0]) === null || _b === void 0 ? void 0 : _b.raw_version;
        }
     ...
     * });
     */
    const getMaxSatisfyingVersion = async function (deviceType, versionOrRange = 'latest', osType) {
        var _a;
        deviceType = await _getNormalizedDeviceTypeSlug(deviceType);
        ...
        return (_a = _getMaxSatisfyingVersion(versionOrRange, osVersions)) !== null && _a !== void 0 ? _a : null;
    };
    /**
     ...
     * });
     */
    const download = (0, util_1.onlyIf)(!isBrowser)(async function (_a) {
        var { deviceType, version = 'latest' } = _a, restOptions = tslib_1.__rest(_a, ["deviceType", "version"]);
        try {
            const slug = await _getNormalizedDeviceTypeSlug(deviceType);
            if (version === 'latest') {
                const foundVersion = (await getAvailableOsVersions(slug)).find((v) => v.osType === OsTypes.DEFAULT);
                if (!foundVersion) {
                    throw new balena_errors_1.BalenaReleaseNotFound('No version available for this device type');
                }
            ...
            return await request.stream({
                method: 'GET',
                url: '/download',
                qs: Object.assign(Object.assign({}, restOptions), { deviceType,
                    version }),
                baseUrl: apiUrl,
            ...
            if ((0, util_1.isNotFoundResponse)(err)) {
     ...
     * });
     */
    const getConfig = async function (slugOrUuidOrId, options) {
        var _a;
        ...
        const { body } = await request.send({
            ...
            url: '/download-config',
            baseUrl: apiUrl,
            body: Object.assign(Object.assign({}, options), { appId: applicationId }),
        });
        return body;
    };
    /**
     ...
     * });
     */
    const getOsUpdateType = async (deviceType, currentVersion, targetVersion) => {
        deviceType = await _getNormalizedDeviceTypeSlug(deviceType);
        return hupActionHelper().getHUPActionType(deviceType, currentVersion, targetVersion);
    };
    /**
     ...
     * });
     */
    const getSupportedOsUpdateVersions = async (deviceType, currentVersion, options) => {
        deviceType = await _getNormalizedDeviceTypeSlug(deviceType);
        ...
        const versions = allVersions.filter((v) => hupActionHelper().isSupportedOsUpdate(deviceType, currentVersion, v));
     ...
     * console.log(result2);
     */
    const isArchitectureCompatibleWith = (osArchitecture, applicationArchitecture) => {
        const compatibleArches = archCompatibilityMap[osArchitecture];
        return (osArchitecture === applicationArchitecture ||
     ...
     * );
     */
    const getSupervisorReleasesForCpuArchitecture = async (cpuArchitectureSlugOrId, options) => {
        const results = await pine.get({
            resource: 'release',
            options: (0, util_1.mergePineOptionsTyped)({
                                ...
                                a: {
                                    ...
                                    is_for__device_type: {
                                        $any: {
                                            ...
                                            $expr: {
                                                dt: {
                                                    is_of__cpu_architecture: typeof cpuArchitectureSlugOrId === 'number'
    ...
    return {
        ...
        _getNormalizedDeviceTypeSlug: _getNormalizedDeviceTypeSlug,
        _getDownloadSize: _getDownloadSize,
