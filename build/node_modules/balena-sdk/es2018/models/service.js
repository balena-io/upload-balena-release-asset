...
sdkInstance, }) => {
        ...
        async getResourceId(id) {
            if (id != null && typeof id === 'object') {
                if (Object.keys(id).length !== 2 ||
                    !('application' in id) ||
                    !('service_name' in id)) {
                    throw new Error(`Unexpected type for id provided in service varModel getResourceId: ${typeof id}`);
                }
                const alternateServiceKey = id;
                const [service] = await getAllByApplication(alternateServiceKey.application, {
                    $select: 'id',
                    $filter: {
                ...
                if (service == null) {
                    throw new errors.BalenaServiceNotFound(alternateServiceKey.service_name);
                }
                return service.id;
            }
            if (typeof id !== 'number') {
    ...
    async function getAllByApplication(slugOrUuidOrId, options = {}) {
        const { service } = await sdkInstance.models.application.get(slugOrUuidOrId, {
            $select: 'id',
            $expand: {
                service: options,
            },
    ...
    const exports = {
        ...
        var: {
            ...
            async getAllByApplication(slugOrUuidOrId, options = {}) {
                ...
                return varModel.getAll((0, util_1.mergePineOptions)({
                    $filter: {
                        service: {
                            $any: {
                                $alias: 's',
