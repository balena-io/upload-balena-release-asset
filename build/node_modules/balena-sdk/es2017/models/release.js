...
const getReleaseModel = function (deps, opts) {
    ...
    require('../util/dependent-resource');
    const builderHelper = (0, once_1.default)(() => {
        const { BuilderHelper } = 
        // eslint-disable-next-line @typescript-eslint/no-require-imports
     ...
     * });
     */
    async function get(commitOrIdOrRawVersion, options = {}) {
        ...
        if ((0, util_1.isId)(commitOrIdOrRawVersion)) {
            const release = await pine.get({
                ...
                options: (0, util_1.mergePineOptions)({}, options),
            });
            if (release == null) {
                throw new errors.BalenaReleaseNotFound(commitOrIdOrRawVersion);
            }
            ...
            if (releases.length === 0) {
                throw new errors.BalenaReleaseNotFound(typeof commitOrIdOrRawVersion === 'string'
                    ? commitOrIdOrRawVersion
                    : `unique pair ${Object.keys(commitOrIdOrRawVersion).join(' & ')}: ${Object.values(commitOrIdOrRawVersion).join(' & ')}`);
            }
            if (releases.length > 1) {
                throw new errors.BalenaAmbiguousRelease(commitOrIdOrRawVersion);
            }
            return releases[0];
        }
    }
     ...
     * });
     */
    async function getWithImageDetails(commitOrIdOrRawVersion, options = {}) {
        ...
        const rawRelease = (await get(commitOrIdOrRawVersion, (0, util_1.mergePineOptions)(baseReleaseOptions, options.release)));
     ...
     * });
     */
    async function getAllByApplication(slugOrUuidOrId, options = {}) {
     ...
     * });
     */
    async function getLatestByApplication(slugOrUuidOrId, options = {}) {
        const [release] = await getAllByApplication(slugOrUuidOrId, (0, util_1.mergePineOptions)({
            $top: 1,
            $filter: {
                status: 'success',
            },
     ...
     * });
     */
    async function createFromUrl(slugOrUuidOrId, urlDeployOptions) {
        const appOptions = {
            $select: 'app_name',
            $expand: {
                organization: {
        ...
        const { app_name, organization } = (await sdkInstance.models.application.get(slugOrUuidOrId, appOptions));
     ...
     *
     */
    async function finalize(commitOrIdOrRawVersion) {
        const { id } = await get(commitOrIdOrRawVersion, { $select: 'id' });
        await pine.patch({
            resource: 'release',
            id,
     ...
     *
     */
    async function setIsInvalidated(commitOrIdOrRawVersion, isInvalidated) {
        const { id } = await get(commitOrIdOrRawVersion, { $select: 'id' });
        await pine.patch({
            resource: 'release',
            id,
     ...
     *
     */
    async function setNote(commitOrIdOrRawVersion, noteOrNull) {
        const { id } = await get(commitOrIdOrRawVersion, { $select: 'id' });
        await pine.patch({
            resource: 'release',
            id,
     ...
     */
    async function setKnownIssueList(commitOrIdOrRawVersion, knownIssueListOrNull) {
        const { id } = await get(commitOrIdOrRawVersion, { $select: 'id' });
        await pine.patch({
            resource: 'release',
            id,
    ...
    const tags = {
         ...
         * });
         */
        async getAllByRelease(commitOrIdOrRawVersion, options = {}) {
            const releaseOpts = {
                $select: 'id',
                $expand: {
                    release_tag: (0, util_1.mergePineOptions)({ $orderby: 'tag_key asc' }, options),
                },
            };
