...
sdkInstance, }, { apiUrl, isBrowser }) {
    ...
    const exports = {
         ...
         * });
         */
        createSetupIntent: async ({ organization, ...extraParams }) => {
            const orgId = await getOrgId(organization);
            const { body } = await request.send({
                method: 'POST',
         ...
         * });
         */
        removeBillingInfo: async (organization) => {
            const orgId = await getOrgId(organization);
            const { body } = await request.send({
                method: 'DELETE',
                url: `/billing/v1/account/${orgId}/info`,
         ...
         * balena.models.billing.updateAccountInfo(orgId, { email: 'hello@balena.io' })
         */
        updateAccountInfo: async (organization, accountInfo) => {
            const orgId = await getOrgId(organization);
            await request.send({
                method: 'PATCH',
                url: `/billing/v1/account/${orgId}`,
         ...
         * });
         */
        changePlan: async (organization, { cycle, ...restPlanChangeOptions }) => {
            const orgId = await getOrgId(organization);
            await request.send({
                method: 'PATCH',
                url: `/billing/v1/account/${orgId}/plan`,
