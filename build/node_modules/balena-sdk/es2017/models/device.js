...
exports.default = exports.DeviceState = void 0;
const tslib_1 = require("tslib");
const device_overall_status_1 = require("../types/device-overall-status");
...
const device_supervisor_api_partial_1 = require("./device.supervisor-api.partial");
const subDays_1 = require("date-fns/subDays");
const MIN_OS_MC = '2.12.0';
const OVERRIDE_LOCK_ENV_VAR = 'RESIN_OVERRIDE_LOCK';
exports.DeviceState = tslib_1.__importStar(require("../types/device-state"));
...
const getDeviceModel = function (deps, opts) {
    ...
    const registerDevice = (0, once_1.default)(() => require('balena-register-device').getRegisterDevice({ request }));
    const { buildDependentResource } = require('../util/dependent-resource');
    const hupActionHelper = (0, once_1.default)(() => require('../util/device-actions/os-update/utils').hupActionHelper);
    const dateUtils = (0, once_1.default)(() => require('../util/date'));
    const batchDeviceOperation = (0, once_1.default)(() => require('../util/request-batching').batchResourceOperationFactory({
        getAll,
    ...
    const getOsUpdateHelper = (0, once_1.default)(async () => {
        const $deviceUrlsBase = await getDeviceUrlsBase();
        const _getOsUpdateHelper = require('../util/device-actions/os-update').getOsUpdateHelper;
    ...
    const tagsModel = buildDependentResource({ pine }, {
        ...
        async getResourceId(uuidOrId) {
            if (typeof uuidOrId !== 'string' && typeof uuidOrId !== 'number') {
                throw new Error(`Unexpected type for uuidOrId provided in device tagsModel getResourceId: ${typeof uuidOrId}`);
            }
    ...
    const configVarModel = buildDependentResource({ pine }, {
        ...
        async getResourceId(uuidOrId) {
            if (typeof uuidOrId !== 'string' && typeof uuidOrId !== 'number') {
                throw new Error(`Unexpected type for uuidOrId provided in device configVarModel getResourceId: ${typeof uuidOrId}`);
            }
            const { id } = await exports.get(uuidOrId, { $select: 'id' });
            return id;
        },
    ...
    const envVarModel = buildDependentResource({ pine }, {
        ...
        async getResourceId(uuidOrId) {
            if (typeof uuidOrId !== 'string' && typeof uuidOrId !== 'number') {
                throw new Error(`Unexpected type for uuidOrId provided in device envVarModel getResourceId: ${typeof uuidOrId}`);
            }
            const { id } = await exports.get(uuidOrId, { $select: 'id' });
            return id;
        },
    ...
    const getDeviceUrlsBase = (0, once_1.default)(async function () {
        if (deviceUrlsBase != null) {
            return deviceUrlsBase;
        }
        return (await sdkInstance.models.config.getAll()).deviceUrlsBase;
    });
    const getAppliedConfigVariableValue = async (uuidOrId, name) => {
        var _a;
        const options = {
            $expand: {
        ...
        const { device_config_variable: [deviceConfig], belongs_to__application: [{ application_config_variable: [appConfig], },], } = (await exports.get(uuidOrId, options));
        return (_a = (deviceConfig !== null && deviceConfig !== void 0 ? deviceConfig : appConfig)) === null || _a === void 0 ? void 0 : _a.value;
    };
    const set = async (uuidOrIdOrArray, body) => {
    ...
    const historyTimeRangeFilterWithGuard = (fromDate, toDate) => {
        let fromDateFilter;
        let toDateFilter;
        if (fromDate != null) {
        ...
        const timeRangeFilter = fromDateFilter || toDateFilter
            ? { created_at: Object.assign(Object.assign({}, fromDateFilter), toDateFilter) }
            : {};
    ...
    async function getAll(options) {
        ...
        const devices = await pine.get({
            resource: 'device',
            options: (0, util_1.mergePineOptions)({ $orderby: 'device_name asc' }, options),
        });
        return devices;
    }
    async function startOsUpdate(uuidOrUuids, targetOsVersion, options = { runDetached: true }) {
        var _a, _b;
        if (!targetOsVersion) {
            throw new errors.BalenaInvalidParameterError('targetOsVersion', targetOsVersion);
        }
        const isDraft = ((_b = (_a = bSemver.parse(targetOsVersion)) === null || _a === void 0 ? void 0 : _a.prerelease.length) !== null && _b !== void 0 ? _b : 0) > 0;
        const getDeviceType = (0, memoizee_1.default)(async (deviceTypeId) => await sdkInstance.models.deviceType.get(deviceTypeId, {
            $select: 'slug',
        }), { primitive: true, promise: true });
        const getAvailableOsVersions = (0, memoizee_1.default)(async (slug, includeDraft) => await sdkInstance.models.os.getAvailableOsVersions(slug, {
            includeDraft,
        }), { primitive: true, promise: true });
        ...
        await batchDeviceOperation()({
            ...
            fn: async (devices, deviceTypeId) => {
                ...
                for (const device of devices) {
                    ...
                    exports._checkOsUpdateTarget(Object.assign(Object.assign({}, device), { is_of__device_type: [dt] }), targetOsVersion);
                    const osVersions = await getAvailableOsVersions(dt.slug, isDraft);
                    if (!osVersions.some((v) => bSemver.compare(v.raw_version, targetOsVersion) === 0)) {
                        throw new errors.BalenaInvalidParameterError('targetOsVersion', targetOsVersion);
                    }
                }
                // use the v2 device actions api for detached updates
                await (0, util_1.limitedMap)(devices, async (device) => {
                    results[device.uuid] = await osUpdateHelper.startOsUpdate(device.uuid, targetOsVersion, options.runDetached === true ? 'v2' : 'v1');
                });
            },
        });
    ...
    const exports = Object.assign(Object.assign({ OverallStatus: device_overall_status_1.DeviceOverallStatus,
        /**
         ...
         * })
         */
        async getAllByApplication(slugOrUuidOrId, options) {
            options !== null && options !== void 0 ? options : (options = {});
            const { id } = await sdkInstance.models.application.get(slugOrUuidOrId, {
                $select: 'id',
            });
            return await getAll((0, util_1.mergePineOptions)({ $filter: { belongs_to__application: id } }, options));
        },
        /**
        ...
        async getAllByOrganization(handleOrId, options) {
            ...
            return await getAll((0, util_1.mergePineOptions)({
                $filter: {
                    belongs_to__application: {
                        $any: {
                            $alias: 'bta',
        ...
        async getWithServiceDetails(uuidOrId, options) {
            options !== null && options !== void 0 ? options : (options = {});
            const device = await exports.get(uuidOrId, (0, util_1.mergePineOptions)({ $expand: (0, device_service_details_1.getCurrentServiceDetailsPineExpand)(true) }, options));
            return (0, device_service_details_1.generateCurrentServiceDetails)(device);
        },
        /**
        ...
        async getByName(name, options) {
            options !== null && options !== void 0 ? options : (options = {});
            const devices = await getAll((0, util_1.mergePineOptions)({ $filter: { device_name: name } }, options));
            if (devices.length === 0) {
        ...
        getLocalIPAddresses: async (uuidOrId) => {
            ...
            return (ip_address !== null && ip_address !== void 0 ? ip_address : '').split(' ');
        }, 
        /**
         ...
         */
        deactivate: async (uuidOrIdOrArray) => {
         ...
         */
        setNote: async (uuidOrIdOrArray, note) => {
         ...
         */
        setCustomLocation: async (uuidOrIdOrArray, location) => {
            await set(uuidOrIdOrArray, {
         ...
         */
        unsetCustomLocation: async (uuidOrIdOrArray) => {
         ...
         */
        move: async (uuidOrIdOrArray, applicationSlugOrUuidOrId) => {
            ...
            const application = (await sdkInstance.models.application.get(applicationSlugOrUuidOrId, applicationOptions));
            const appCpuArchSlug = application.is_for__device_type[0].is_of__cpu_architecture[0].slug;
            const deviceOptions = {
            ...
            await batchDeviceOperation()({
                ...
                fn: async (devices) => {
                    for (const device of devices) {
                        const isCompatibleMove = sdkInstance.models.os.isArchitectureCompatibleWith(device.is_of__device_type[0].is_of__cpu_architecture[0].slug, appCpuArchSlug);
                        if (!isCompatibleMove) {
                            throw new errors.BalenaInvalidDeviceType(`Incompatible application: ${applicationSlugOrUuidOrId}`);
                        }
                    }
                    await pine.patch({
                        resource: 'device',
        ...
        } }, (0, device_supervisor_api_partial_1.getSupervisorApiHelper)(deps, opts)), { 
        /**
         ...
         * });
         */
        getSupervisorTargetState: async (uuidOrId, version = 2) => {
         ...
         *
         */
        getSupervisorTargetStateForApp: async (slugOrUuidOrId, release) => {
            const { uuid } = await sdkInstance.models.application.get(slugOrUuidOrId, {
                $select: 'uuid',
            });
            const { body } = await request.send({
                url: `/device/v3/fleet/${uuid}/state/?releaseUuid=${release !== null && release !== void 0 ? release : ''}`,
                baseUrl: apiUrl,
            });
            return body;
        }, 
         ...
         * });
         */
        async register(applicationSlugOrUuidOrId, uuid, deviceTypeSlug) {
            ...
            const [{ id: userId }, apiKey, application, deviceType] = await Promise.all([
                ...
                sdkInstance.models.application.get(applicationSlugOrUuidOrId, applicationOptions),
            ...
            if (deviceType != null) {
                const isCompatibleParameter = sdkInstance.models.os.isArchitectureCompatibleWith(deviceType.is_of__cpu_architecture[0].slug, application.is_for__device_type[0].is_of__cpu_architecture[0].slug);
                if (!isCompatibleParameter) {
                    throw new errors.BalenaInvalidDeviceType(`Incompatible device type: ${deviceTypeSlug}`);
                }
            }
            return await registerDevice().register({
                ...
                uuid,
                deviceType: (deviceType !== null && deviceType !== void 0 ? deviceType : application.is_for__device_type[0]).slug,
         ...
         * });
         */
        generateDeviceKey: async (uuidOrId, keyName, keyDescription, keyExpiryDate) => {
            const deviceId = (await sdkInstance.models.device.get(uuidOrId, { $select: 'id' })).id;
            const { body } = await request.send({
                method: 'POST',
         ...
         */
        enableDeviceUrl: async (uuidOrIdOrArray) => {
            await set(uuidOrIdOrArray, {
         ...
         */
        disableDeviceUrl: async (uuidOrIdOrArray) => {
            await set(uuidOrIdOrArray, {
        ...
        async enableLocalMode(uuidOrId) {
            ...
            (0, local_mode_1.checkLocalModeSupported)(device);
            await exports.configVar.set(device.id, local_mode_1.LOCAL_MODE_ENV_VAR, '1');
        }, 
        /**
         ...
         */
        enableLockOverride: (uuidOrId) => configVarModel.set(uuidOrId, OVERRIDE_LOCK_ENV_VAR, '1'), 
        /**
         ...
         */
        disableLockOverride: (uuidOrId) => configVarModel.set(uuidOrId, OVERRIDE_LOCK_ENV_VAR, '0'), 
        /**
        ...
        hasLockOverride: async (uuidOrId) => {
            return ((await getAppliedConfigVariableValue(uuidOrId, OVERRIDE_LOCK_ENV_VAR)) === '1');
        }, 
        /**
         ...
         */
        async grantSupportAccess(uuidOrIdOrArray, expiryTimestamp) {
         ...
         */
        revokeSupportAccess: async (uuidOrIdOrArray) => {
            await set(uuidOrIdOrArray, {
         ...
         * })
         */
        getOsVersion: (device) => (0, device_os_version_1.getDeviceOsSemverWithVariant)(device), 
        /**
        ...
        getTargetReleaseHash: async (uuidOrId) => {
            ...
            const { should_be_running__release } = (await exports.get(uuidOrId, deviceOptions));
            return (_a = should_be_running__release[0]) === null || _a === void 0 ? void 0 : _a.commit;
        }, 
        /**
         ...
         * });
         */
        pinToRelease: async (uuidOrIdOrArray, fullReleaseHashOrId) => {
            const getRelease = (0, memoizee_1.default)(async (appId) => {
                const releaseFilterProperty = (0, util_1.isId)(fullReleaseHashOrId)
                    ? 'id'
                    : 'commit';
         ...
         * });
         */
        trackApplicationRelease: async (uuidOrIdOrArray) => {
            await set(uuidOrIdOrArray, {
         ...
         * });
         */
        setSupervisorRelease: async (uuidOrIdOrArray, supervisorVersionOrId) => {
            const releaseFilterProperty = (0, util_1.isId)(supervisorVersionOrId)
                ? 'id'
                : 'raw_version';
            const getRelease = (0, memoizee_1.default)(async (cpuArchId) => {
                const [supervisorRelease] = await sdkInstance.models.os.getSupervisorReleasesForCpuArchitecture(cpuArchId, {
                    $top: 1,
                    $select: 'id',
                    $filter: {
            ...
            await batchDeviceOperation()({
                ...
                fn: async (devices) => {
                    devices.forEach((device) => {
                        (0, device_os_version_1.ensureVersionCompatibility)(device.supervisor_version, device_supervisor_api_partial_1.MIN_SUPERVISOR_MC_API, 'supervisor');
                        (0, device_os_version_1.ensureVersionCompatibility)(device.os_version, MIN_OS_MC, 'host OS');
                    });
                    const devicesByDeviceType = (0, util_1.groupByMap)(devices, (device) => device.is_of__device_type[0].is_of__cpu_architecture.__id);
                    await Promise.all([...devicesByDeviceType.entries()].map(async ([cpuArchId, devicesOfType]) => {
        ...
        _checkOsUpdateTarget({ uuid, is_of__device_type, is_online, os_version, os_variant, }, targetOsVersion) {
            ...
            const currentOsVersion = (_b = (0, device_os_version_1.getDeviceOsSemverWithVariant)({
                os_version,
                os_variant,
            })) !== null && _b !== void 0 ? _b : os_version;
        ...
        tags: {
            ...
            async getAllByApplication(slugOrUuidOrId, options) {
                ...
                return await tagsModel.getAll((0, util_1.mergePineOptions)({
                    $filter: {
                        device: {
                            $any: {
                                $alias: 'd',
        ...
        configVar: {
            ...
            async getAllByApplication(slugOrUuidOrId, options) {
                ...
                return await configVarModel.getAll((0, util_1.mergePineOptions)({
                    $filter: {
                        device: {
                            $any: {
                                $alias: 'd',
        ...
        envVar: {
            ...
            async getAllByApplication(slugOrUuidOrId, options) {
                ...
                return await envVarModel.getAll((0, util_1.mergePineOptions)({
                    $filter: {
                        device: {
                            $any: {
                                $alias: 'd',
        ...
        serviceVar: {
             ...
             * });
             */
            async get(uuidOrId, serviceNameOrId, key) {
                const { id: deviceId } = await exports.get(uuidOrId, { $select: 'id' });
                            ...
                            service_install: {
                                $any: {
                                    ...
                                    $expr: {
                                        si: {
                                            device: deviceId,
                                            installs__service: typeof serviceNameOrId === 'number'
                                                ? serviceNameOrId
                                                : {
                                                    $any: {
                                                        $alias: 'is',
             ...
             * });
             */
            async set(uuidOrId, serviceNameOrId, key, value) {
                value = String(value);
                let deviceFilter;
                if ((0, util_1.isId)(uuidOrId)) {
                    deviceFilter = uuidOrId;
                }
                else if ((0, util_1.isFullUuid)(uuidOrId)) {
                    deviceFilter = {
                        $any: {
                            $alias: 'd',
                            $expr: {
             ...
             * });
             */
            async remove(uuidOrId, serviceNameOrId, key) {
                const { id: deviceId } = await exports.get(uuidOrId, { $select: 'id' });
                await pine.delete({
                    resource: 'device_service_environment_variable',
                    options: {
        ...
        history: {
             ...
             */
            async getAllByDevice(uuidOrId, _a = {}) {
                var { fromDate = (0, subDays_1.subDays)(new Date(), DEFAULT_DAYS_OF_REQUESTED_HISTORY), toDate } = _a, options = tslib_1.__rest(_a, ["fromDate", "toDate"]);
                let $filter = historyTimeRangeFilterWithGuard(fromDate, toDate);
                if ((0, util_1.isId)(uuidOrId)) {
                    $filter = Object.assign(Object.assign({}, $filter), { tracks__device: uuidOrId });
                }
                else if ((0, util_1.isFullUuid)(uuidOrId)) {
                    $filter = Object.assign(Object.assign({}, $filter), { uuid: uuidOrId });
                }
                else {
                    throw new errors.BalenaInvalidParameterError('uuidOrId', uuidOrId);
                }
             ...
             *
             */
            async getAllByApplication(slugOrUuidOrId, _a = {}) {
                var { fromDate = (0, subDays_1.subDays)(new Date(), DEFAULT_DAYS_OF_REQUESTED_HISTORY), toDate } = _a, options = tslib_1.__rest(_a, ["fromDate", "toDate"]);
                const { id: applicationId } = await sdkInstance.models.application.get(slugOrUuidOrId, {
                    $select: 'id',
                });
                return await pine.get({
                    resource: 'device_history',
                    options: (0, util_1.mergePineOptions)({
                        $filter: Object.assign(Object.assign({}, historyTimeRangeFilterWithGuard(fromDate, toDate)), { belongs_to__application: applicationId }),
                    }, options),
                });
            },
        } });
