...
exports.HUPActionHelper = exports.HUPActionError = exports.actionsConfig = void 0;
...
const getVariant = (semver) => {
    const semverExtraParts = [...semver.build, ...semver.prerelease];
    return ['dev', 'prod'].find((variant) => semverExtraParts.includes(variant));
};
class HUPActionError extends typed_error_1.TypedError {
}
exports.HUPActionError = HUPActionError;
class HUPActionHelper {
    constructor(actionsConfig = config_1.actionsConfig) {
     ...
     */
    getHUPActionType(deviceType, currentVersion, targetVersion) {
        var _a;
        ...
        if (targetVariant != null &&
            ...
            throw new HUPActionError('Updates cannot be performed between development and production balenaOS variants');
        }
        ...
        if (fromMajor === 1) {
            switch (toMajor) {
                ...
                case 2:
                    ...
                    break;
                default:
                    throw new HUPActionError(`This update request cannot be performed from ${currentVersion} to ${targetVersion}`);
            }
        }
        else {
        ...
        const deviceActions = (_a = actionsConfig.deviceTypes[deviceType]) !== null && _a !== void 0 ? _a : {};
        if (defaultActions[actionName] == null &&
            deviceActions[actionName] == null) {
            throw new HUPActionError(`This update request cannot be performed on '${deviceType}'`);
        }
        const { minSourceVersion, targetMajorVersion, minTargetVersion, minTakeoverVersion, maxTargetVersion, } = Object.assign(Object.assign(Object.assign({}, actionsConfig.actions[actionName]), defaultActions[actionName]), deviceActions[actionName]);
        if (bSemver.lt(currentVersion, minSourceVersion)) {
            throw new HUPActionError(`Current OS version must be >= ${minSourceVersion}`);
        }
        ...
        if (targetMajorVersion &&
            ...
            throw new HUPActionError(`Target OS version must be of major version ${targetMajorVersion}`);
        }
        if (bSemver.lt(targetVersion, minTargetVersion)) {
            throw new HUPActionError(`Target OS version must be >= ${minTargetVersion}`);
        }
        if (maxTargetVersion && bSemver.gte(targetVersion, maxTargetVersion)) {
            throw new HUPActionError(`Target OS version must be < ${maxTargetVersion}`);
        }
        if (actionName === 'balenahup' && minTakeoverVersion != null) {
            ...
            if (bSemver.lt(noVariantVersion, minTakeoverVersion) &&
